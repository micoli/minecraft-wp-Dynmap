...
DynMap.prototype = {
	...
	initialize: function() {
			...
			$.each(world.maps, function(mapindex, map) {
				...
				map.element = $('<li/>')
					...
					.append($('<a/>')
							...
							.addClass('maptype')
							.css({ backgroundImage: 'url(' + (map.options.icon || (me.options.url.webroot+'images/block_' + mapindex + '.png')) + ')' })
		...
		var configset = { };
		if(me.options.inlineComponents!=undefined){
			$.each(me.options.inlineComponents, function(index, configuration){
		...
		$.each(configset, function(type, configlist) {
			...
			loadjs(me.options.url.webroot+'js/' + type + '.js', function() {
	...
	selectWorldAndPan: function(world, location, completed) {
		var me = this;
		if (typeof(world) === 'String' || typeof(world) === 'string') { world = me.worlds[world]; }
		if (me.world === world) {
			if(location) {
	...
	addPlayer: function(update) {
		...
		var menuitem = player.menuitem = $('<li/>')
			...
			.append(playerIconContainer = $('<span/>')
					.addClass('playerIcon')
					.append($('<img/>').attr({ src: dynmap.options.url.webroot+'images/player_face.png' }))
